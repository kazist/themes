...
class StringModification {
    ...
    function variablizeFirstCap($word) {
        $word = ucfirst(strtolower($this->variablize($word)));
        return $word;
    }

    function variablizeLow($word) {
        $word = strtolower($this->variablize($word));
        return $word;
    }

    function singleWordLow($word) {
        $word = $this->camelize($word);
        $word = strtolower($word);
        return $word;
    }

    function singleWordFirstCap($word) {
        $word = ucfirst($this->singleWordLow($word));
        return $word;
    }

    function seoword($word) {

        $word = $this->underscore($word);
        $word = trim(str_replace('_', '-', $word), '-');

        return $word;
    }

    function friendlyUrl($word) {
        return $this->seoword($word);
    }
    ...
    public function getArrayToString($array, $html = '') {

        try {
            $tmparray = json_decode(json_encode($array), true);

            if (!empty($tmparray)) {

                $html .= '<table style="width:100%;border-spacing: 0px;border-collapse: collapse;">';

                foreach ($tmparray as $key => $item) {
                    $html .= '<tr>';
                    if (is_array($item) && !empty($item)) {
                        $html .= '<td style="border: 1px solid #DDD;"><b>' . $key . '</b></td>';
                        $html .= '<td  style="border: 1px solid #DDD; padding:5px;">';
                        $html .= $this->getArrayToString($item);
                        $html .= '</td>';
                    } else {
                        if (!is_numeric($key)) {
                            $html .= '<td style="border: 1px solid #DDD;"><b>' . $key . '</b></td>';
                            $html .= '<td style="border: 1px solid #DDD;">' . $item . '</td>';
                        } else {
                            $html .= '<td colspan="3" style="border: 1px solid #DDD;">' . $item . '</td>';
                        }
                    }
                    $html .= '</tr>';
                }
