...
var fc = jQuery.fullCalendar = { version: "1.6.4" };
...
function Calendar(element, options, eventSources) {
	...
	function _changeView(newViewName) {
		...
		currentView = new fcViews[newViewName](
			jQuery("<div class='fc-view fc-view-" + newViewName + "' style='position:relative'/>")
				.appendTo(content),
			...
			if (fetchID == currentFetchID) {
				if (events) {
					...
					if (options.eventDataTransform) {
						events = jQuery.map(events, options.eventDataTransform);
					}
					if (source.eventDataTransform) {
						events = jQuery.map(events, source.eventDataTransform);
					}
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			...
			if (url) {
				...
				// retrieve any outbound GET/POST jQuery.ajax data from the options
				var customData;
				if (jQuery.isFunction(source.data)) {
				...
				var data = jQuery.extend({}, customData || {});
...
function parseDate(s, ignoreTimezone) { // ignoreTimezone defaults to true
	...
	if (typeof s == 'string') {
		if (s.match(/^\d+(\.\d+)?jQuery/)) { // a UNIX timestamp
...
function parseISO8601(s, ignoreTimezone) { // ignoreTimezone defaults to false
	...
	var m = s.match(/^([0-9]{4})(-([0-9]{2})(-([0-9]{2})([T ]([0-9]{2}):([0-9]{2})(:([0-9]{2})(\.([0-9]+))?)?(Z|(([-+])([0-9]{2})(:?([0-9]{2}))?))?)?)?)?jQuery/);
	if (!m) {
		return null;
	}
...
function BasicView(element, calendar, viewName) {
	...
	function buildEventContainer() {
		daySegmentContainer =
			jQuery("<div class='fc-event-container' style='position:absolute;z-index:8;top:0;left:0'/>")
				.appendTo(element);
	}
	...
	function buildTable() {
		...
		table = jQuery(html).appendTo(element);
		...
		bodyCells.each(function(i, _cell) {
			...
			trigger('dayRender', t, date, jQuery(_cell));
		});
	...
	function dayClick(ev) {
		if (!opt('selectable')) { // if selectable, SelectionManager will worry about dayClick
			var date = parseISO8601(jQuery(this).data('date'));
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		if (opt('allDaySlot')) {
			...
			daySegmentContainer = jQuery([]); // in jQuery 1.4, we can just do jQuery()
		...
		s +=
			"</tbody>" +
			"</table>";
		slotTable = jQuery(s).appendTo(slotContainer);
	...
	function buildDayTable() {
		...
		dayTable = jQuery(html).appendTo(element);
...
function View(element, calendar, viewName) {
	...
	function triggerEventDestroy() {
		jQuery.each(eventElementCouples, function(i, couple) {
	...
	(function() {
		...
		for (var dayIndex=0, cellIndex=0; dayIndex<7; dayIndex++) {
			...
			isHiddenDayHash[dayIndex] = jQuery.inArray(dayIndex, hiddenDays) != -1;
...
function DayEventRenderer() {
	...
	function _renderDayEvents(events, doAppend, doRowHeights) {
		...
		var renderContainer = doAppend ? jQuery("<div/>") : finalContainer;
			...
			hoverListener.start(function(cell, origCell) {
				if (cell) {
					...
					if (dayDelta) {
						...
						helpers = jQuery(helpers); // turn array into a jQuery object
